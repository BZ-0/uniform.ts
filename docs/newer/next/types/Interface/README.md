[**@fest-lib/uniform v0.1.31**](../../../../README.md)

***

[@fest-lib/uniform](../../../../README.md) / newer/next/types/Interface

# newer/next/types/Interface

## Enumerations

- [WReflectAction](enumerations/WReflectAction.md)
- [WStatus](enumerations/WStatus.md)
- [WType](enumerations/WType.md)

## Interfaces

- [AtomicsConfig](interfaces/AtomicsConfig.md)
- [ChannelMessage](interfaces/ChannelMessage.md)
- [ChannelMeta](interfaces/ChannelMeta.md)
- [ConnectionOptions](interfaces/ConnectionOptions.md)
- [DataAssetEnvelope](interfaces/DataAssetEnvelope.md)
- [Observer](interfaces/Observer.md)
- [PendingRequest](interfaces/PendingRequest.md)
- [PortConfig](interfaces/PortConfig.md)
- [RTCConfig](interfaces/RTCConfig.md)
- [SharedWorkerConfig](interfaces/SharedWorkerConfig.md)
- [Subscribable](interfaces/Subscribable.md)
- [Subscriber](interfaces/Subscriber.md)
- [Subscription](interfaces/Subscription.md)
- [TransferableConfig](interfaces/TransferableConfig.md)
- [TransportCapabilities](interfaces/TransportCapabilities.md)
- [WError](interfaces/WError.md)
- [WReflectDescriptor](interfaces/WReflectDescriptor.md)
- [WReq](interfaces/WReq.md)
- [WResp](interfaces/WResp.md)
- [WSuccess](interfaces/WSuccess.md)

## Type Aliases

- [ChannelState](type-aliases/ChannelState.md)
- [InvokerHandler](type-aliases/InvokerHandler.md)
- [MessageHandler](type-aliases/MessageHandler.md)
- [Producer](type-aliases/Producer.md)
- [ResponderFn](type-aliases/ResponderFn.md)
- [SendFn](type-aliases/SendFn.md)
- [TransportTarget](type-aliases/TransportTarget.md)
- [TransportType](type-aliases/TransportType.md)
