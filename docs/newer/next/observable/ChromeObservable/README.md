[**@fest-lib/uniform v0.1.31**](../../../../README.md)

***

[@fest-lib/uniform](../../../../README.md) / newer/next/observable/ChromeObservable

# newer/next/observable/ChromeObservable

## Classes

- [ChromeExternalObservable](classes/ChromeExternalObservable.md)
- [ChromePortObservable](classes/ChromePortObservable.md)
- [ChromeRuntimeObservable](classes/ChromeRuntimeObservable.md)
- [ChromeTabsObservable](classes/ChromeTabsObservable.md)

## Interfaces

- [ChromeMessage](interfaces/ChromeMessage.md)
- [ChromeObservableOptions](interfaces/ChromeObservableOptions.md)
- [PortInfo](interfaces/PortInfo.md)

## Variables

- [ChromeObservableFactory](variables/ChromeObservableFactory.md)

## Functions

- [createChromeRequestHandler](functions/createChromeRequestHandler.md)
