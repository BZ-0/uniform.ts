[**@fest-lib/uniform v0.1.31**](../../../../README.md)

***

[@fest-lib/uniform](../../../../README.md) / newer/next/transport/AtomicsTransport

# newer/next/transport/AtomicsTransport

## Classes

- [AtomicsBuffer](classes/AtomicsBuffer.md)
- [AtomicsRingBuffer](classes/AtomicsRingBuffer.md)
- [AtomicsTransport](classes/AtomicsTransport.md)

## Interfaces

- [AtomicsChannelPair](interfaces/AtomicsChannelPair.md)
- [AtomicsMessage](interfaces/AtomicsMessage.md)
- [AtomicsTransportConfig](interfaces/AtomicsTransportConfig.md)
- [RingBufferConfig](interfaces/RingBufferConfig.md)

## Variables

- [AtomicsTransportFactory](variables/AtomicsTransportFactory.md)

## Functions

- [createAtomicsChannelPair](functions/createAtomicsChannelPair.md)
- [createWorkerAtomicsTransport](functions/createWorkerAtomicsTransport.md)
